<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            @if($user->profile_picture)
                <div class="mt-2">
                    ...
                    <img src="{{ asset('storage/' . $user->profile_picture) }}" alt="{{ $user->name }}" class="mt-1 h-24 w-24 object-cover rounded-full">
                </div>
            @endif
        </div>
        ...
        @if($user->isAdmin())
            <div>
                <p class="text-sm text-gray-600">{{ __('Account Type:') }} <span class="font-medium">{{ __('Administrator') }}</span></p>
            </div>
        @endif

        <div>
            <x-input-label for="wallet_balance" :value="__('Wallet Balance')" />
            <p class="mt-1 text-sm text-gray-600">{{ $user->getFormattedBalanceAttribute() }}</p>
        </div>
